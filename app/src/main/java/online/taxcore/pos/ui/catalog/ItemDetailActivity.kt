...
class ItemDetailActivity : BaseActivity() {
    ...
    override fun onResume() {
        super.onResume()
        useESDC = pref.getBoolean(PrefConstants.USE_ESDC_SERVER, false)
    ...
    private fun initToolbar() {
        ...
        supportActionBar?.title = if (isInCreateMode) "Add Item" else "Edit Item"
    }
    ...
    private fun populateItemFields(item: Item?) {
        item?.let {
            ...
            activity_detail_plu_input_price.setText(it.price.roundTo2DecimalPlaces())
        ...
        activity_detail_plu_input_price.onTextChanged {
            ...
            if (isNumber(it)) {
                updatedFieldsMap["price"] =
                    (it.roundTo2DecimalPlaces().toDouble() != item?.price?.roundTo2DecimalPlaces()
                        ?.toDouble())
            }
    ...
    private fun isNumber(it: String): Boolean {
        return try {
            it.toDouble() > 0 && it.toDouble().roundTo2DecimalPlaces().isNotEmpty()
