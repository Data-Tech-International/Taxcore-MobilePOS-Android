...
class InvoiceAdapter(
    ...
    private fun openEditQuantityDialog(ctx: Context, currentItem: Item, position: Int) {
        val quantityText = currentItem.quantity.roundToDecimalPlaces(3)
        ...
        MaterialDialog(ctx).show {
            ...
            qtyInput.filters = arrayOf<InputFilter>(DecimalDigitsInputFilter(6, 3))
            ...
            getCustomView().dialogQtyInput.onTextChanged { inputText ->
                ...
                if (isInputValid) {
                    ...
                    qtyLayout.error = "Quantity can not be less than 0.010"
                }
    ...
    private fun openConfirmRemoveItemDialog(
        ctx: Context,
        item: Item,
        position: Int,
        onItemRemoved: (item: Item) -> Unit
    ) {

        MaterialDialog(ctx).show {
            title(text = "Are you sure that you want to remove item from invoice?")
    ...
    fun bind(item: Item, validTaxes: List<String>) {
        ...
        item.tax.forEach { tax ->
            val chipView = inflater.inflate(
                ...
                itemView.invoiceItemTaxLabelsChipGroup,
                false
            ) as Chip
        ...
        val totalPrice = (item.price * item.quantity).roundTo2DecimalPlaces()
        val quantity = item.quantity.roundToDecimalPlaces(3)
