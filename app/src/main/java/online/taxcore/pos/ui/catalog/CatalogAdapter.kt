...
class CatalogAdapter(
    ...
    private fun confirmRemoveItem(position: Int, item: Item, update: () -> Unit) {
        MaterialDialog(context).show {
            title(text = "Are you sure that you want to remove item `${item.name}` from catalog?")
    ...
    private fun remove(position: Int, item: Item, update: () -> Unit) {
        ...
        Toast.makeText(context, "Item `${item.name}` removed", Toast.LENGTH_SHORT).show()
    ...
    inner class PluViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: Item, validTaxes: List<String>) {
            ...
            // Price
            val itemPrice = item.price.toString().toDouble().roundTo2DecimalPlaces()
            itemView.item_plu_title.text = itemTitle

            val priceText =
                "Unit price: <font color='#FF5722'><b>${itemPrice}</b></font>".fromHtml()
